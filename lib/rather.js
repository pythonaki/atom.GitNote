...
function isImageFile(filename) {
  return imgFileTypes.some(ext => extname(filename).toLowerCase() === ext);
}


function isMarkdownFile(filename) {
  return mdExts.some(ext => extname(filename).toLowerCase() === ext);
}


function isHtmlFile(filename) {
  return htmlExts.some(ext => extname(filename).toLowerCase() === ext);
}
...
const gitnoteUri = {
  ...
  format(format, ignores = []) {
    const ff = {
      ...
      auth: format.auth,
      hostname: (format.repository)? format.repository : '//',
