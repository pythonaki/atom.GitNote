  ...
  _createClass(GitNote, [{
    ...
    value: function dictionary() {
      ...
      return this.update().then(function (updated) {
        if (!_this._unpackedDic) {
          ...
          //   dics.push(item);
          // }
          for (var id in _this._dic) {
            var note = _this._dic[id];
            var heads = note.heads;
            for (var i in heads) {
              heads[i].id = note.id;
              heads[i].file = note.file;
              heads[i].path = _path2.default.resolve(_this.workDir, GitNote.env.NOTE_DIRNAME, note.id, note.file);
            }
            dics.push(heads);
          }
    ...
    value: function updateDictionary() {
      ...
      return this._repo.getStatus().then(function (statuses) {
        var filters = statuses.filter(function (status) {
          return GitNote.isNoteFile(status.path());
        });
    ...
    value: function _isChangedFiles(statuses) {
      ...
      var promises = statuses.map(function (status) {
        var id = GitNote.getId(status.path());
    ...
    key: 'workDir',
    get: function get() {
      return _path2.default.resolve(this._repo.workdir());
    }
  }], [{
    key: 'wasInited',
    ...
    key: 'getId',
    value: function getId(notePath) {
      return _path2.default.basename(notePath, _path2.default.extname(notePath));
    }
  }, {
    key: 'isNoteFile',
    value: function isNoteFile(notePath) {
      ...
      if (dirname === GitNote.getId(notePath) && GitNote.env.ABLE_EXTS.some(function (item) {
    ...
    // static createHeadId(headline) {
    //   return 'head-' + $4.sha1(headline);
    // }

  }, {
    key: 'createHashName',
    value: function createHashName(headline) {
      var reg = /[\{\}\[\]\/?.,;:|\)*~`!^_+<>@\#$%&\\\=\(\'\"]/g;
      var decoded = (0, _entities.decodeHTML)(headline);
      return decoded.toLowerCase().replace(reg, '').replace(/[\s]/g, '-');
    }
    ...
    key: 'createName',
    value: function createName(id, uri) {
      return 'f-' + id + '-' + _fourdollar2.default.sha1(uri).slice(0, 8) + _path2.default.extname(uri);
    }

    // 임의로 이름 지정. (렌덤)

  }, {
    key: 'createRandomName',
    value: function createRandomName(id) {
      return 'f-' + id + '-' + _fourdollar2.default.randomID(16);
    }
  }, {
    key: '_openDic',
                    ...
                    if (stats.isDirectory()) {
                      return _fsExtra2.default.readdir(_path2.default.resolve(notePath, name)).then(function (items) {
                        var ables = items.filter(function (item) {
                          return GitNote.isNoteFile(_path2.default.resolve(notePath, name, item));
                        });
    ...
    // {
    //   id: id,                          // require
    //   file: file.md,                   // require
    //   heads: [
    //     {
    //       hash: hash,           // option
    //       level: level,                // require
    //       headline: headline,          // require
    //       fullHeadline: fullHeadline   // require
    //     }
    //   ]
    // }
      ...
      var getDicItems = noteFiles.map(function (noteFile) {
        return _fsExtra2.default.readFile(noteFile, { encoding: 'utf-8' }).then(function (content) {
          ...
          renderer.heading = function (text, level) {
            ...
            heads.push({
              hash: heads.length == 0 ? undefined : GitNote.createHashName(text),
              level: heads.length == 0 ? 0 : level,
              headline: text,
          ...
          if (heads.length === 0) {
            dic[id].heads = [{
              level: 0,
              headline: 'untitled: ' + id,
              fullHeadline: 'untitled: ' + id
            }];
          } else {
            dic[id].heads = heads;
          }
